...
describe('DThorswapV2Router02', () => {
  ...
  it('getAmountOut', async () => {
    ...
    await expect(router.getAmountOut(bigNumberify(0), bigNumberify(100), bigNumberify(100), bigNumberify(30))).to.be.revertedWith(
      'DThorswapV2Library: INSUFFICIENT_INPUT_AMOUNT'
    )
  ...
  it('getAmountIn', async () => {
    ...
    await expect(router.getAmountIn(bigNumberify(0), bigNumberify(100), bigNumberify(100), bigNumberify(30))).to.be.revertedWith(
      'DThorswapV2Library: INSUFFICIENT_OUTPUT_AMOUNT'
    )
  ...
  it('getAmountsOut', async () => {
    ...
    await expect(router.getAmountsOut(bigNumberify(2), [token0.address])).to.be.revertedWith(
      'DThorswapV2Library: INVALID_PATH'
    )
...
describe('fee-on-transfer tokens', () => {
  ...
  beforeEach(async function() {
    ...
    // make a DTT<>WVET pair
    await fixture.factoryV2.createPair(DTT.address, WVET.address)
    const pairAddress = await fixture.factoryV2.getPair(DTT.address, WVET.address)
    pair = new Contract(pairAddress, JSON.stringify(IDThorswapV2Pair.abi), provider).connect(wallet)
  })
  ...
  async function addLiquidity(DTTAmount: BigNumber, WVETAmount: BigNumber) {
    await DTT.approve(router.address, MaxUint256)
    await router.addLiquidityVET(DTT.address, DTTAmount, DTTAmount, WVETAmount, wallet.address, MaxUint256, {
      ...overrides,
      value: WVETAmount
    })
  }
  ...
  it('removeLiquidityVETSupportingFeeOnTransferTokens', async () => {
    ...
    const WVETInPair = await WVET.balanceOf(pair.address)
    const liquidity = await pair.balanceOf(wallet.address)
    const totalSupply = await pair.totalSupply()
    const NaiveDTTExpected = DTTInPair.mul(liquidity).div(totalSupply)
    const WVETExpected = WVETInPair.mul(liquidity).div(totalSupply)
  ...
  describe('swapExactTokensForTokensSupportingFeeOnTransferTokens', () => {
    ...
    // WVET -> DTT
    it('WVET -> DTT', async () => {
      await WVET.deposit({ value: amountIn }) // mint WVET
      await WVET.approve(router.address, MaxUint256)

      await router.swapExactTokensForTokensSupportingFeeOnTransferTokens(
        amountIn,
        0,
        [WVET.address, DTT.address],
        wallet.address,
        MaxUint256,
        overrides
      )
