...
import './SafeMath.sol';
import './DThorswapV2Library.sol';

// library containing some math for dealing with the liquidity shares of a pair, e.g. computing their exact value
// in terms of the underlying tokens
library DThorswapV2LiquidityMathLibrary {
    ...
    function getReservesAfterArbitrage(
        ...
        (reserveA, reserveB) = DThorswapV2Library.getReserves(factory, tokenA, tokenB);

        require(reserveA > 0 && reserveB > 0, 'DThorswapV2ArbitrageLibrary: ZERO_PAIR_RESERVES');

        uint swapFee = DThorswapV2Library.getSwapFee(factory, tokenA, tokenB);
        ...
        if (aToB) {
            uint amountOut = DThorswapV2Library.getAmountOut(amountIn, reserveA, reserveB, swapFee);
            reserveA += amountIn;
            reserveB -= amountOut;
        } else {
            uint amountOut = DThorswapV2Library.getAmountOut(amountIn, reserveB, reserveA, swapFee);
            reserveB += amountIn;
            reserveA -= amountOut;
        }
    }
    ...
    function computeLiquidityValue(
        ...
        uint256 platformFee, // bool feeOn,
        uint kLast
    ...
    function getLiquidityValue(
        ...
        (uint256 reservesA, uint256 reservesB) = DThorswapV2Library.getReserves(factory, tokenA, tokenB);
        IDThorswapV2Pair pair = IDThorswapV2Pair(DThorswapV2Library.pairFor(factory, tokenA, tokenB));
        /*
        bool feeOn = IDThorswapV2Factory(factory).feeTo() != address(0);
