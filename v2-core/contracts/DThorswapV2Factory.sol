...
import './DThorswapV2Pair.sol';
import './libraries/Ownable.sol';

contract DThorswapV2Factory is IDThorswapV2Factory, Ownable {
    ...
    constructor(uint _defaultSwapFee, uint _defaultPlatformFee, address _platformFeeTo, address _defaultRecoverer) public {
        require(_platformFeeTo != address(0)); // 'DThorswapV2: PLATFORMFEETO_ZERO_ADDRESS'
        require(_defaultSwapFee >= MIN_SWAP_FEE && _defaultSwapFee <= MAX_SWAP_FEE); // 'DThorswapV2: INVALID_SWAP_FEE'
        require(_defaultPlatformFee <= MAX_PLATFORM_FEE); // 'DThorswapV2: INVALID_PLATFORM_FEE'
    ...
    function createPair(address tokenA, address tokenB) external returns (address pair) {
        require(tokenA != tokenB, 'DThorswapV2: IDENTICAL_ADDRESSES');
        (address token0, address token1) = tokenA < tokenB ? (tokenA, tokenB) : (tokenB, tokenA);
        require(token0 != address(0), 'DThorswapV2: ZERO_ADDRESS');
        require(getPair[token0][token1] == address(0), 'DThorswapV2: PAIR_EXISTS'); // single check is sufficient
        bytes memory bytecode = type(DThorswapV2Pair).creationCode;
        bytes32 salt = keccak256(abi.encodePacked(token0, token1));
        assembly {
            pair := create2(0, add(bytecode, 32), mload(bytecode), salt)
        }
        IDThorswapV2Pair(pair).initialize(token0, token1, defaultSwapFee, defaultPlatformFee);
    ...
    function getPairInitHash() public pure returns(bytes32){
        bytes memory rawInitCode = type(DThorswapV2Pair).creationCode;
        return keccak256(abi.encodePacked(rawInitCode));
    }

    function setPlatformFeeTo(address _platformFeeTo) external onlyOwner {
        require(_platformFeeTo != address(0), 'ZERO_ADDRESS');
        emit PlatformFeeToChanged(platformFeeTo, _platformFeeTo);
        platformFeeTo = _platformFeeTo;
    }

    function setDefaultSwapFee(uint _swapFee) external onlyOwner {
        require(_swapFee >= MIN_SWAP_FEE && _swapFee <= MAX_SWAP_FEE, "INVALID_SWAP_FEE");
    ...
    function setDefaultPlatformFee(uint _platformFee) external onlyOwner {
        require(_platformFee <= MAX_PLATFORM_FEE, "INVALID_PLATFORM_FEE");
    ...
    function setSwapFeeForPair(address _pair, uint _swapFee) external onlyOwner {
        require(_swapFee >= MIN_SWAP_FEE && _swapFee <= MAX_SWAP_FEE, "INVALID_SWAP_FEE");
        IDThorswapV2Pair(_pair).setSwapFee(_swapFee);
    }

    function setPlatformFeeForPair(address _pair, uint _platformFee) external onlyOwner {
        require(_platformFee <= MAX_PLATFORM_FEE, "INVALID_PLATFORM_FEE");
        IDThorswapV2Pair(_pair).setPlatformFee(_platformFee);
    }
}
