...
import './interfaces/IDThorswapV2Pair.sol';
import './DThorswapV2ERC20.sol';
import './libraries/Math.sol';
import './libraries/UQ112x112.sol';
import './interfaces/IERC20.sol';
import './interfaces/IDThorswapV2Factory.sol';
import './interfaces/IDThorswapV2Callee.sol';

contract DThorswapV2Pair is IDThorswapV2Pair, DThorswapV2ERC20 {
    ...
    modifier lock() {
        require(unlocked == 1, 'DThorswapV2: LOCKED');
        unlocked = 0;
        _;
        unlocked = 1;
    }
    ...
    modifier onlyFactory() {
        require(msg.sender == factory, 'DThorswapV2: FORBIDDEN');
        _;
    }
    ...
    function _safeTransfer(address token, address to, uint value) private {
        ...
        require(success && (data.length == 0 || abi.decode(data, (bool))), 'DThorswapV2: TRANSFER_FAILED');
    }
    ...
    function _update(uint balance0, uint balance1, uint112 _reserve0, uint112 _reserve1) private {
        require(balance0 <= uint112(-1) && balance1 <= uint112(-1), 'DThorswapV2: OVERFLOW');
    ...
    function _mintFee(uint112 _reserve0, uint112 _reserve1) private returns (bool feeOn) {
        ...
        if (feeOn) {
            uint _sqrtOldK = Math.sqrt(_kLast);
            if (_sqrtOldK != 0) {
                ...
                if (_sqrtNewK > _sqrtOldK) {
                    ...
                    address platformFeeTo = IDThorswapV2Factory(factory).platformFeeTo();
    ...
    function mint(address to) external lock returns (uint liquidity) {
        ...
        require(liquidity > 0, 'DThorswapV2: INSUFFICIENT_LIQUIDITY_MINTED');
        _mint(to, liquidity);
    ...
    function burn(address to) external lock returns (uint amount0, uint amount1) {
        ...
        require(amount0 > 0 && amount1 > 0, 'DThorswapV2: INSUFFICIENT_LIQUIDITY_BURNED');
    ...
    function swap(uint amount0Out, uint amount1Out, address to, bytes calldata data) external lock {
        require(amount0Out > 0 || amount1Out > 0, 'DThorswapV2: INSUFFICIENT_OUTPUT_AMOUNT');
        (uint112 _reserve0, uint112 _reserve1,) = getReserves(); // gas savings
        require(amount0Out < _reserve0 && amount1Out < _reserve1, 'DThorswapV2: INSUFFICIENT_LIQUIDITY');
        ...
        require(to != _token0 && to != _token1, 'DThorswapV2: INVALID_TO');
        if (amount0Out > 0) _safeTransfer(_token0, to, amount0Out); // optimistically transfer tokens
        if (amount1Out > 0) _safeTransfer(_token1, to, amount1Out); // optimistically transfer tokens
        if (data.length > 0) IDThorswapV2Callee(to).dthorswapV2Call(msg.sender, amount0Out, amount1Out, data);
        balance0 = IERC20(_token0).balanceOf(address(this));
        balance1 = IERC20(_token1).balanceOf(address(this));
        }
        uint amount0In = balance0 > _reserve0 - amount0Out ? balance0 - (_reserve0 - amount0Out) : 0;
        uint amount1In = balance1 > _reserve1 - amount1Out ? balance1 - (_reserve1 - amount1Out) : 0;
        require(amount0In > 0 || amount1In > 0, 'DThorswapV2: INSUFFICIENT_INPUT_AMOUNT');
        ...
        require(balance0Adjusted.mul(balance1Adjusted) >= uint(_reserve0).mul(_reserve1).mul(10000**2), 'DThorswapV2: K');
        }
    ...
    function recoverToken(address token, uint amount) external {
        require(token != token0 && token != token1, "INVALID_TOKEN");
        address recoverer = IDThorswapV2Factory(factory).defaultRecoverer();
        require(recoverer != address(0));
        IERC20(token).transfer(recoverer, amount);
    }
}
