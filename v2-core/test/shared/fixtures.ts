...
import { BigNumber, bigNumberify } from 'ethers/utils' // DThor
import { deployContract } from 'ethereum-waffle'

// import { expandTo18Decimals } from './utilities'
import { MAX_UINT_128 } from './utilities' // DThor

import ERC20 from '../../build/ERC20.json'
import DThorswapV2Factory from '../../build/DThorswapV2Factory.json'
...
export async function factoryFixture(_: Web3Provider, [wallet]: Wallet[]): Promise<FactoryFixture> {
  ...
  const factory = await deployContract(wallet, DThorswapV2Factory, [defaultSwapFee, defaultPlatformFee, platformFeeTo, wallet.address], overrides)
...
export async function pairFixture(provider: Web3Provider, [wallet]: Wallet[]): Promise<PairFixture> {
  ...
  // Setup initial liquidity of pair's tokens; 10000 x 10^8  originally used in DThor tests, this
  // is expanded for overflow testing of new platformFee tests to max-uint 128bit.
  const tokenSupply: BigNumber = MAX_UINT_128; // or expandTo18Decimals(10000)
  ...
  const pair = new Contract(pairAddress, JSON.stringify(DThorswapV2Pair.abi), provider).connect(wallet)
