...
describe('DThorswapV2Pair', () => {
  ...
  it('swap:gas', async () => {
    ...
    expect(receipt.gasUsed).to.eq(75080) // 73462
  })
  ...
  it('platformFeeTo:off', async () => {
    ...
    expect(receipt.gasUsed).to.satisfy( function(gas: number) {
      return verifyGas( gas, [63043, 97219, 105059, 105080], "platformFee off swap gas" );
    })
  ...
  it('platformFeeTo:on', async () => {
    ...
    expect(receipt.gasUsed).to.satisfy( function(gas: number) {
      return verifyGas(gas, [63043, 105059, 105080], "Swap gas cost");
    })
    ...
    expect(burnReceipt.gasUsed, "Check burn op gas cost").to.satisfy( function(gas: number) {
      return verifyGas( gas, [135736, 135747, 135856, 177752, 177763], "Burn gas cost" );
    })
    ...
    // The (inverted) target max variance of 0.02% of DThorswap platform fee to DThorswapV2.
    ...
    // Compare 1/6 DThorswapV2 fee, using 0.1667 DThorswap Platform fee: run check to confirm ~ 0.02% variance.
    const token0ExpBalDThorswapV2: BigNumber = bigNumberify( '249501683697445' )
    const token0ExpBalDThorswap: BigNumber = bigNumberify( '249551584034184' )
    const token0Variance: number = token0ExpBalDThorswapV2.div(token0ExpBalDThorswap.sub(token0ExpBalDThorswapV2)).toNumber();
    expect(token0Variance, "token 0 variance from uniswap v2 fee" ).to.be.within(minInverseVariance, maxInverseVariance)

    // Compare 1/6 DThorswapV2 fee, using 0.1667 DThorswap Platform fee: run check to confirm ~ 0.02% variance.
    const token1ExpBalDThorswapV2: BigNumber = bigNumberify( '250000187312969' )
    const token1ExpBalDThorswap: BigNumber = bigNumberify( '250050187350431' )
    const token1Variance: number = token1ExpBalDThorswapV2.div(token1ExpBalDThorswap.sub(token1ExpBalDThorswapV2)).toNumber();
    ...
    expect(await token0.balanceOf(pair.address), "Token 0 balance of pair").to.eq(bigNumberify(1000).add(token0ExpBalDThorswap))
    expect(await token1.balanceOf(pair.address), "Token 1 balance of pair").to.eq(bigNumberify(1000).add(token1ExpBalDThorswap))
  })
  ...
  {
    // Constants from DThorswapV2Pair _calcFee
    ...
    // The algorithm from DThorswapV2Pair _calcFee
  ...
  comparisonTestCases.forEach((platformFeeTestCase, i) => {
    it(`compareCalcPlatformFee:${i}`, async () => {
      ...
      const lDThorswapFeeResult: BigNumber = calcPlatformFee( platformFee, token0InitialBalance, token1InitialBalance, token0FinalBalance, token1FinalBalance )
      ...
      // Calculate variance
      const lDelta : number = lUniswapFeeResult - lDThorswapFeeResult.toNumber();
      const lDeltaPct : number = (lUniswapFeeResult == 0) ? 0 : (lDelta * 100 / lUniswapFeeResult);

      // Validate that the DThorswap result is within 1 integer value in all cases.
      expect( lDelta, 'DThorswap equation validation' ).to.satisfy(
      ...
      const lComparisonRecord: ComparisonRecord = { 
        ...
        DThorswapFee: lDThorswapFeeResult.toString(),
 ...
 /**
  ...
  swapAndPlatformFeeTestCases.forEach((swapAndPlatformTestCase, i) => {
    it(`platformFeeRange:${i}`, async () => {
      ...
      return verifyGas( gas, [63031, 63043, 63052, 63064, 105047, 105068, 105080], "swap gas fee" ); })
      ...
      expect(burnReceipt.gasUsed, "burn gas fee").to.satisfy( 
        function(gas: number) { return verifyGas( gas, [145110, 145121, 145230, 157137, 157246, 187126, 187137, 187246], "burn gas fee" ); })
      ...
      const token0ExpBalDThorswap: BigNumber = bigNumberify( expectedPlatformFee )
      expect(await token0.balanceOf(pair.address), "Token 0 balance of pair").to.satisfy( 
        function(a:BigNumber) { return closeTo(a, bigNumberify(1000).add(token0ExpBalDThorswap)) } )

      const token1ExpBalDThorswap: BigNumber = bigNumberify( expectedPlatformFee )
      expect(await token1.balanceOf(pair.address), "Token 1 balance of pair").to.satisfy(
        function(a:BigNumber) { return closeTo(a, bigNumberify(1000).add(token1ExpBalDThorswap)) } )
    })
  })
  ...
  it('basicOverflow', async () => {
    ...
    await expect( pair.mint(wallet.address, overrides), 'mint with too much balance' ).to.be.revertedWith( 'DThorswapV2: OVERFLOW' )
    ...
    await expect( pair.swap(bigNumberify(1), 0, wallet.address, '0x', overrides), 'swap with too much balance').to.be.revertedWith( 'DThorswapV2: OVERFLOW' )
  })
  ...
  it('recoverToken:AddressZero', async () => {
    await expect(pair.recoverToken(token2.address, '0')).to.be.revertedWith('DThorswapV2: RECOVERER_ZERO_ADDRESS')
  ...
  it('recoverToken:noAmount', async () => {
    ...
    await pair.recoverToken(token2.address, '0')
    expect(await token2.balanceOf(recoveryAddress)).to.eq(0)    
  })
  ...
  it('recoverToken:base' , async () => {
    ...
    const recoveryAddress = other.address
    await pair.recoverToken(token2.address, token2Amount)
